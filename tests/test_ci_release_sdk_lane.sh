...
require_job_contains \
  ...
  "nightly must build the real Ghostty CLI helper on macOS 15"

require_job_contains \
  "$NIGHTLY_FILE" \
  "build-sign-notarize-nightly" \
  'runs-on: ${{ vars.MACOS_RUNNER_26 || '\''warp-macos-26-arm64-6x'\'' }}' \
  "nightly must build the app on macOS 26"
...
for workflow in "$CI_FILE" "$NIGHTLY_FILE" "$RELEASE_FILE"; do
...
echo "PASS: release and nightly app builds use macOS 26 SDK with a macOS 15-built Ghostty CLI helper"
